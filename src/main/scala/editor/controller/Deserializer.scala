            ...
            .map(jsCell =>
              ...
              EnumHelper.toCellTypes(cellType).get match {
                case CellTypes.Mover =>
                  SetupMoverCell(position, EnumHelper.toOrientation((jsCell \ "orientation").as[String]).get, true)
                case CellTypes.Block =>
                  SetupBlockCell(position, EnumHelper.toPush((jsCell \ "push").as[String]).get, true)
                case CellTypes.Enemy => SetupEnemyCell(position, true)
                case CellTypes.Rotator =>
                  SetupRotatorCell(position, EnumHelper.toRotation((jsCell \ "rotation").as[String]).get, true)
                case CellTypes.Wall => SetupWallCell(position, true)
                case CellTypes.Generator =>
                  SetupGeneratorCell(position, EnumHelper.toOrientation((jsCell \ "orientation").as[String]).get, true)
