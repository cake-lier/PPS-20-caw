...
object EditorView {
  ...
  /* An abstract implementation of EditorView factorizing common aspects between its subclasses. */
  private abstract class AbstractEditorView(
    scene: Scene,
    ...
    override def drawState(state: EditorModelState): Unit = Platform.runLater(() =>
      boardView match {
        ...
        case None =>
          val newBoardView: EditorBoardView = EditorBoardView(scene.width.value, scene.height.value, state, this)
    ...
    override def addPlayableArea(topLeftCorner: Position, downRightCorner: Position): Unit =
      controller.addPlayableArea(
        topLeftCorner,
        (downRightCorner.x - topLeftCorner.x + 1, downRightCorner.y - topLeftCorner.y + 1)
      )
