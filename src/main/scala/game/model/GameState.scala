...
/** The state of the game, an entity collecting all information about the state of the [[GameModel]], which runs the current game.
  ...
  * to happen next. These pieces of information regards the current [[Level]] being played, its initial and current state, its
  * index, if it has been completed or not, if there is a next level to be played and whether or not an enemy died in the last
  * game update. It must be constructed through its companion object.
  */
trait GameState {

  /** Returns the state of the [[Level]] currently being played from which the game begins updating it. */
  ...
  /** Returns the index of the [[Level]] currently being played. If the player is not playing the default levels, its value is
    * always zero.
    */
  val currentLevelIndex: Int

  /** Returns whether or not there is a [[Level]] to be played after the current one. */
...
object GameState {
  ...
  /** Returns a new instance of the [[GameState]] trait given the initial and current state of the [[Level]] currently being
    * played for the game updates, the index of the [[Level]] currently being played, if there is another [[Level]] to be played
    * after the current one, if an [[it.unibo.pps.caw.common.model.cell.EnemyCell]] has died after the last update and if the
    * current [[Level]] has been completed or not.
    *
    * @param levelInitialState
    *   the initial state of the [[Level]] currently being played with respect to the game updates
    * @param levelCurrentState
    *   the current state of the [[Level]] currently being played after all the game updates already happened
    * @param currentLevelIndex
    *   the index of the [[Level]] currently being played
    * @param hasNextLevel
    *   whether or not there is another [[Level]] to be played after the current one
    * @param didEnemyDie
    ...
    *   whether or not the current [[Level]] has been completed
    * @return
  ...
  extension (state: GameState) {
    ...
    /** Copy constructor of the [[GameState]] trait which creates a new instance of [[GameState]] copying the already created one
      * and modifying its properties with the given values for the initial and current state of the [[Level]] currently being
      * played for the game updates, the index of the [[Level]] currently being played, if there is another [[Level]] to be played
      * after the current one, if an [[it.unibo.pps.caw.common.model.cell.EnemyCell]] has died after the last update and if the
      * current [[Level]] has been completed or not.
      *
