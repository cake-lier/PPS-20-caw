...
/** An error that the [[ApplicationController]] can get into during the normal execution of its function.
  ...
  * occured and the given function could not be correctly performed.
  */
enum ApplicationControllerError(val message: String) {

  /** The error that occurs when a [[it.unibo.pps.caw.common.model.Level]] could not be loaded. */
  case CouldNotLoadLevel extends ApplicationControllerError(message = "An error has occured, could not load level")

  /** The error that occurs when the [[it.unibo.pps.caw.common.storage.Settings]] could not be saved. */
  case CouldNotSaveSettings extends ApplicationControllerError(message = "An error has occured, could not save settings")
}
