...
trait WithDimensionsWord {
  ...
  /** Allows to use the "withDimensions" word in a sentence written in the DSL, specifying the [[Dimensions]] of an area. It
...
object WithDimensionsWord {
  ...
  /** Returns a new instance of the [[WithDimensionsWord]] trait. It needs a function which can consume the [[Dimensions]] that
    * the user specifies through the use of this "withDimensions" word and which can return an [[AtWord]] that the user must then
    * use for completing the sentence.
    *
    * @param fun
    *   the function for consuming the [[Dimensions]] the user specifies through this "withDimensions" word and which returns the
