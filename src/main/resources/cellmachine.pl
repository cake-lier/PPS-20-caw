...
% state. If the coordinates does not point to a "mover right" cell, the predicate simply evaluates to "no". No ordering is
% required for the cells in the board and the "empty" cells must not be represented.
mover_top_next_state(B, X, Y, NB) :- member(cell(mover_top, X, Y), B),
...
rotator_right_next_state(B, X, Y, NB) :- member(cell(_, rotate_right, X, Y), B), !, rotate_right(B, X, Y, NB).
...
% the empty cells must not be present.
rotator_left_next_state(B, X, Y, NB) :- member(cell(_, rotate_left, X, Y), B), !, rotate_left(B, X, Y, NB).
