...
% to the right if it is one that can always move to the right, like an "enemy" or a "delete" cell, or if it is a cell that can
...
% the board.
last_index_right(_, T, X, _, X) :- (T = enemy; T = delete), !.
...
% If the last cell met while moving them is a "delete" cell, the one previous to it is removed. No ordering is required for the
% cells in the board.
move_right([], _, _, _, _, []).
...
% removed as is removed the one previous to the "enemy" cell. If the last cell which can be moved them is a "delete" cell, the one
...
% to the left if it is one that can always move to the left, like an "enemy" or a "delete" cell, or if it is a cell that can move
...
% board.
last_index_left(_, T, X, _, X) :- (T = enemy; T = delete), !.
...
% are then moved. If no action can be performed, the input board is given as output. Any cell in the board that is not affected by
% this rule is ignored and copied in the result as is. If the last cell which can be moved is an "enemy" cell, the cell is
% removed as is removed the one previous to the "enemy" cell. If the last cell which can be moved is a "delete" cell, the one
...
% move to the bottom if it is one that can always move to the bottom, like an "enemy" or a "delete" cell, or if it is a cell that
...
last_index_down(_, T, _, Y, Y) :- (T = enemy; T = delete), !.
...
% the last cell met while moving them is a "delete" cell, the one previous to it is is removed. No ordering is required for the
% cells in the board.
move_down([], _, _, _, _, []).
...
% move to the top if it is one that can always move to the top, like an "enemy" or a "delete" cell, or if it is a cell that can
...
% board.
last_index_top(_, T, _, Y, Y) :- (T = enemy; T = delete), !.
...
% adjacent cell is an "enemy" cell, the board is left unchanged. If the adjacent cell is a "delete" cell, it is generated
...
generator_right_next_state(B, X, Y, _, B) :- member(cell(_, generator_right, X, Y), B),
                                             ...
                                             !.
generator_right_next_state(B, X, Y, M, [cell(M, delete, X2, Y) | NB]) :- member(cell(_, generator_right, X, Y), B),
                                                                         X1 is X - 1,
                                                                         member(cell(_, delete, X1, Y), B),
                                                                         X2 is X + 1,
...
generator_left_next_state(B, X, Y, M, B) :- member(cell(_, generator_left, X, Y), B),
                                            ...
                                            member(cell(_, enemy, X1, Y), B),
                                            !.
generator_left_next_state(B, X, Y, M, [cell(M, delete, X2, Y) | NB]) :- member(cell(_, generator_left, X, Y), B),
                                                                        X1 is X + 1,
                                                                        member(cell(_, delete, X1, Y), B),
                                                                        X2 is X - 1,
                                                                        drop_first(B, cell(_, _, X2, Y), NB),
...
% adjacent cell is an enemy cell, the board is unchanged. If the adjacent cell is a "delete" cell, it is generated
...
generator_down_next_state(B, X, Y, M, B) :- member(cell(_, generator_down, X, Y), B),
                                            ...
                                            !.
generator_down_next_state(B, X, Y, M, [cell(M, delete, X, Y2) | NB]) :- member(cell(_, generator_down, X, Y), B),
                                                                        Y1 is Y - 1,
                                                                        member(cell(_, delete, X, Y1), B),
                                                                        Y2 is Y + 1,
...
generator_top_next_state(B, X, Y, M, B) :- member(cell(_, generator_top, X, Y), B),
                                           ...
                                           member(cell(_, enemy, X, Y1), B),
                                           !.
generator_top_next_state(B, X, Y, M, [cell(M, delete, X, Y2) | NB]) :- member(cell(_, generator_top, X, Y), B),
                                                                       Y1 is Y + 1,
                                                                       member(cell(_, delete, X, Y1), B),
                                                                       Y2 is Y - 1,
                                                                       drop_first(B, cell(_, _, X, Y2), NB),
